...
void verify(const std::string& name, const std::vector<int>& input, const std::vector<int>& expected) {
    ...
    const std::vector<int> actual = radixSort(input);
    if (actual != expected) throw std::runtime_error(name + " produced the wrong result");
    if (input != original) throw std::runtime_error(name + " mutated its input");
}
...
int main() {
    ...
    verify("reverse order", {500, 40, 3, 2, 1}, {1, 2, 3, 40, 500});
    verify("duplicates", {21, 11, 21, 12}, {11, 12, 21, 21});
    verify("negative values", {-12, 5, -8, 0}, {-12, -8, 0, 5});
    ...
    std::cout << "C++ Radix Sort tests passed\n";
    return 0;
}
