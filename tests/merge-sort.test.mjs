...
import { mergeSort } from "../algorithms/sorting/merge-sort/merge-sort.mjs";
...
for (const { name, input, expected } of cases) {
  test(`Merge Sort handles ${name}`, () => {
    const original = [...input];
    assert.deepEqual(mergeSort(input), expected);
    assert.deepEqual(input, original, "input should not be mutated");
  });
}

test("Merge Sort agrees with the platform sort oracle for deterministic random inputs", () => {
  let seed = 59;
  const next = () => {
  ...
  for (let round = 0; round < 30; round += 1) {
    ...
    assert.deepEqual(mergeSort(input), [...input].sort((a, b) => a - b));
  }
});
