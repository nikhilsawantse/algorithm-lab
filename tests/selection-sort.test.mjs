...
import { selectionSort } from "../algorithms/sorting/selection-sort/selection-sort.mjs";
...
for (const { name, input, expected } of cases) {
  test(`Selection Sort handles ${name}`, () => {
...
test("Selection Sort agrees with the platform sort oracle for deterministic random inputs", () => {
  let seed = 31;
  const next = () => {
  ...
  for (let round = 0; round < 30; round += 1) {
    ...
    assert.deepEqual(selectionSort(input), [...input].sort((a, b) => a - b));
  }
});
